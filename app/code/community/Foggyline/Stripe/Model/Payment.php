...
class Foggyline_Stripe_Model_Payment extends Mage_Payment_Model_Method_CC
{
	...
	public function capture(Varien_Object $payment, $amount)
	{
		...
		try {
			$charge = Stripe_Charge::create([
				...
				'description' => sprintf('#%s, %s', $order->getIncrementId(),
				$order->getCustomerEmail())
			]);
		} catch (Exception $e) {
			$this->debugData($e->getMessage());
			Mage::throwException(Mage::helper('foggline_stripe')->__('payment
				capturing error.'));
		}
	...
	public function isAvailable($quote = null)
	{
		...
		return $this->getConfigData('api_key', ($quote ? $quote->getStoreId() :
			null)) && parent::isAvailable($quote);
	}
